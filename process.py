...
# def split_train_dev():
#     with open("/home/akashp/145/OAG-AQA/data/kddcup/dpr/train_with_hn.json", "r") as f:
#         data = json.load(f)
#     print(len(data))
...
def split_train_dev_test():
    with open("/home/akashp/145/OAG-AQA/data/kddcup/dpr-old/train_with_hn.json", "r") as f:
        data = json.load(f)
    print(len(data))

    n_len = len(data)
    n_train = int(n_len * 0.7)
    n_dev = (n_len - n_train) // 2
    data_train = data[:n_train]
    data_dev = data[n_train:n_train + n_dev]
    data_test = data[n_train + n_dev:]
    ...
    with open(data_dir + "test.json", "w", encoding="utf-8") as f:
        json.dump(data_test, f, indent=4, ensure_ascii=False)
        
    


def gen_dpr_passages_tsv():
    with open("data/AQA/pid_to_title_abs_new.json") as rf:
        paper_dict = json.load(rf)
    
    wf = open("data/kddcup/dpr/candidate_papers.tsv", "w")
...
#     wf = open("data/kddcup/dpr/qa_valid_dpr.tsv", "w")
#     with open("data/AQA/qa_valid_wo_ans.txt") as rf:
#         for line in tqdm(rf):
...
def gen_dpr_valid_input():
    wf = open("data/kddcup/dpr/qa_valid_dpr.tsv", "w")
    with open("data/kddcup/dpr/test.json") as rf:
        data = json.load(rf)
